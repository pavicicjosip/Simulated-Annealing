...
float f(float z, float theta, float k) {
  ...
  return ((1)/(std::pow(theta, k) * std::tgamma(k))) * std::pow(z, k - 1) * std::exp(-z/theta);
}

float update_tehta(const std::vector<float>& ms, float k) {
  int N = ms.size();
  float sum = std::accumulate(ms.begin(), ms.end(), 0.0f);
  return ((1)/(k*N))*sum;
}
...
float update_k(const std::vector<float>& ms) {
  int N = ms.size();
  float sum = std::accumulate(ms.begin(), ms.end(), 0.0f);
  float sumOfLogarithms = std::accumulate(ms.begin(), ms.end(), 0.0f,
        [](float sum, float value) {
            return sum + log(value);
        });
  float q = std::log((1/N)*sum) - (1/N)*sumOfLogarithms;

  return (3 - q + std::sqrt((std::pow(q - 3, 2) + 24 * q)))/(12 * q);
}

TEXT_FILE ETSA(VRPInstance& instance, int num_multistarts) {
    ...
    while (T > T_final) {
      for(int i = 0; i < N; ++i) {
        ...
        if (delta < 0) {
          ...
          float r = dis(gen);
          float Gamma = f(delta/T, instance.theta, instance.k);
          if (r < Gamma) {
            if(instance.ms.size() == m) {
              instance.ms.erase(instance.ms.begin());
            }
            instance.ms.push_back(delta);

            instance.theta = update_tehta(instance.ms, instance.k);
            instance.k = update_k(instance.ms);
